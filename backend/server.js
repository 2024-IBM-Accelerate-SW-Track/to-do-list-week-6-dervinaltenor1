...
async function initDB () {
  ...
  try {
      ...
      const dbExists = await client.headDatabase({ db: todoDBName }).catch(err => {
        if (err.status !== 404) {
          throw err;
        }
      });

      if (dbExists && dbExists.status === 200) {
        console.log(`Database "${todoDBName}" already exists, no need to create a new one.`);
      } else {
